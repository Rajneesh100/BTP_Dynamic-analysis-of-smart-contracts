...
contract PRIVYstaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The block number when PRIVYstaking reward ends.
    uint256 public bonusEndBlock;

    // The block number when PRIVYstaking reward starts.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && tokenStaked != 0) {
            ...
            uint256 PRIVYstakingReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(PRIVYstakingReward.mul(PRECISION_FACTOR).div(tokenStaked));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 PRIVYstakingReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(PRIVYstakingReward.mul(PRECISION_FACTOR).div(tokenStaked));
        lastRewardBlock = block.number;
    }
