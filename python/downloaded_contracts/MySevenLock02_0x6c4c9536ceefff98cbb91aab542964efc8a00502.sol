...
// File: contracts/SevenLocker/Lock/FullMath.sol
...
// File: contracts/SevenLocker/Lock/IUniswapV2Factory.sol
...
// File: contracts/SevenLocker/Lock/IUniswapV2Pair.sol
...
// File: contracts/SevenLocker/Lock/IUniswapV2Router02.sol
...
// File: contracts/SevenLocker/Lock/Ownable.sol
...
// File: contracts/SevenLocker/Lock/Locker.sol
...
contract MySevenLock02 is ISevenLockNew, Pausable, Ownable {
    ...
    struct FeeStruct {
        ...
        address referralToken; // token the referrer must hold to qualify as a referrer
        uint256 referralHold; // balance the referrer must hold to qualify as a referrer
        uint256 referralDiscountEthFee; // discount on flatrate fees for using a valid referral address
        address feeReceiver; // receive fee whenever user locks or edits lock
        address revShareReceiver; // profit share contract address
        uint256 revSharePercentageFee; // Percentage of fees charged designated for revenue sharing
    }

    // ID padding from SevenLock v1, as there is a lack of a pausing mechanism
    ...
    constructor(address _referralToken, address _feeReceiver, address _revShareAddress) {
        gFees.ethFee = 0.02 ether; // 0.02 eth
        gFees.ethEditFee = 0.02 ether; // 0.02 eth
        gFees.referralToken = _referralToken;
        gFees.referralHold = 100 ether; // 100 tokens
        gFees.referralDiscountEthFee = 0.01 ether; // 0.01 eth
        gFees.feeReceiver = _feeReceiver;
        gFees.revShareReceiver = _revShareAddress;
        gFees.revSharePercentageFee = 25; // 25% 
    }
    ...
    function setFees(
        ...
        address _revShareReceiver,
        uint256 _revSharePercentageFee
    ) public onlyOwner {
        gFees.ethFee = ethFee;
        gFees.ethEditFee = ethEditFee;
        gFees.referralDiscountEthFee = referralDiscountEthFee;
        gFees.feeReceiver = _feeReceiver;
        gFees.revShareReceiver = _revShareReceiver;
        gFees.revSharePercentageFee = _revSharePercentageFee;
    }
    ...
    function lock(
        ...
        uint256 revShare = validFee * gFees.revSharePercentageFee / 100;

        payable(gFees.revShareReceiver).sendValue(revShare);

        payable(gFees.feeReceiver).sendValue(validFee - revShare);
    ...
    function editLock(
        ...
        require(msg.value == gFees.ethEditFee, "SERVICE FEE");

        uint256 revShare = gFees.ethEditFee * gFees.revSharePercentageFee / 100;

        payable(gFees.revShareReceiver).sendValue(revShare);

        payable(gFees.feeReceiver).sendValue(gFees.ethEditFee - revShare);
