...
// File: contracts/watch raffle v2/bit_raffle.sol
...
contract BitRaffle {
    ...
    constructor(
        ...
        require(_maxTicketsPerParticipant <= _maxTickets, "Tickets per Participant exceeding Total Tickets");
        manager = _manager;
        raffle = RaffleInfo(
            _name,
            _image,
        ...
        priceFeedETH = AggregatorV3Interface(
            0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419 // ETH/USD on ETH
    ...
    function calculateCommission(uint256 tickets, uint256 price) internal pure returns (uint256) {
        uint256 commission = 0;
        if (tickets >= 1 && tickets <= 5) commission = price.mul(5).div(100);
        else if (tickets <= 10)  commission = price.mul(10).div(100);
        else if (tickets <= 20) commission = price.mul(15).div(100);
        else if (tickets > 20) commission = price.mul(20).div(100);
        return commission;
    }

    function distributeTickets(string memory email, address participant, uint256 numberOfTickets) internal {
        for (uint256 i = 0; i < numberOfTickets; i++) {
            participants.push(participant);
        }

        raffle.totalTicketsSold = raffle.totalTicketsSold.add(numberOfTickets);
        userInfoMap[participant] = UserInfo(email, userInfoMap[participant].ticketsPurchased.add(numberOfTickets));
    }

    function validateTicketRequirements(uint256 numberOfTickets) internal view {
    ...
    function enterUsingUSDT(string memory email, uint256 numberOfTickets, address referrer) public notClosed {
        validateTicketRequirements(numberOfTickets);

        uint256 ticketPrice = (raffle.ticketPrice * (numberOfTickets)) * 10 ** (ERC20(usdtAddress).decimals());
        uint256 commission = 0;
        ...
        if (referrer != address(0)) {
            ...
            if (affiliate.affiliateAddress == address(0)) affiliate.affiliateAddress = referrer; // Create a new affiliate if the referrer does not exist

            commission = calculateCommission(numberOfTickets, ticketPrice);
            ERC20(usdtAddress).safeTransferFrom(msg.sender, affiliate.affiliateAddress, commission);
        ...
        ERC20(usdtAddress).safeTransferFrom(msg.sender, manager, remainingAmount);

        distributeTickets(email, msg.sender, numberOfTickets);
    ...
    function enterUsingEth(string memory email, uint256 numberOfTickets, address referrer) external payable {
        validateTicketRequirements(numberOfTickets);

        uint256 ticketPrice = raffle.ticketPrice.mul(numberOfTickets);
        uint256 totalValue = usdToEth().mul(ticketPrice);
        require(msg.value >= totalValue, "Not enough price paid");

        uint256 commission = 0;
        if (referrer != address(0)) {
            Affiliate storage affiliate = affiliates[referrer];
            // Create a new affiliate if the referrer does not exist
            if (affiliate.affiliateAddress == address(0)) affiliate.affiliateAddress = referrer;
        
            // Calculate the commission based on the number of tickets
            commission = calculateCommission(numberOfTickets, totalValue);

            payable(affiliate.affiliateAddress).transfer(commission);
            affiliate.ticketsSold = affiliate.ticketsSold.add(numberOfTickets);
            affiliate.totalCommission = affiliate.totalCommission.add(commission);
        }

        uint256 remainingAmount = totalValue.sub(commission);
        payable(manager).transfer(remainingAmount);
    ...
    function withdraw(ERC20 token, address to, uint256 amount) public onlyManager {
        require(amount <= token.balanceOf(address(this)), "Insufficient balance");
        token.transfer(to, amount);
    }

    function updateToken(ERC20 token) public onlyManager {
        usdtAddress = token;
    }
}
...
// File: contracts/watch raffle v2/bit_factory.sol
...
contract BitRaffleFactory is Ownable {
    using SafeERC20 for ERC20;
    ERC20 public usdtAddress;
    ...
    function withdraw(ERC20 token, address to, uint256 amount) public onlyOwner {
        require(amount <= token.balanceOf(address(this)), "Insufficient balance");
        token.transfer(to, amount);
    }
    ...
    function allWinners() external view returns (Winners[] memory) {
        ...
        for (uint256 i = 0; i < raffles.length; i++) {
            bool raffleClosed = BitRaffle(raffles[i]).getRaffle().raffleClosed;
            if (raffleClosed) {
                address[] memory winnersAddr = BitRaffle(raffles[i]).getWinners();
    ...
    function updateToken(ERC20 token) public onlyOwner {
        usdtAddress = token;
    }
}