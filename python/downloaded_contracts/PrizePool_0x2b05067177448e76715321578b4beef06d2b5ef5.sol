...
contract PrizePool is Ownable {
    ...
    function distributePrizes(address firstPlaceWallet, address secondPlaceWallet, address thirdPlaceWallet) public onlyOwner {
        ...
        (bool sentFirst, ) = payable(firstPlaceWallet).call{value: firstPrize}("");
        require(sentFirst, "Failed to send Ether to first place");

        // Send 30% to the second place wallet
        (bool sentSecond, ) = payable(secondPlaceWallet).call{value: secondPrize}("");
        require(sentSecond, "Failed to send Ether to second place");

        // Send 20% to the third place wallet
        (bool sentThird, ) = payable(thirdPlaceWallet).call{value: thirdPrize}("");
