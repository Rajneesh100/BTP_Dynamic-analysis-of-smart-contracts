// File: contracts/interfaces/IAssetTokenFactory.sol
...
// File: contracts/interfaces/IEurPriceFeed.sol
...
// File: contracts/assetToken/authorization/AssetTokenAuthorizationOpen.sol
...
/// @author Swarm Markets
/// @title Swarm Authorization for AssetToken Token Contract - Open
/// @notice Contract to perform validation on swarm ecosystem
contract AssetTokenAuthorizationOpen is AccessControlUpgradeable {
    ...
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "AssetTokenAuth: only DEFAULT_ADMIN");
        _;
    }

    /// @notice Check if the passed address is not zero
    modifier zeroAddressCheck(address account) {
        require(account != address(0), "AssetTokenAuth: passed address = 0");
        _;
    }
    ...
    function isAccountAuthorized(address _account) external view zeroAddressCheck(_account) returns (bool) {
        require(_account != address(0), "AssetTokenAuth: _account == 0");
    ...
    function securityAssetValidation(address _account, uint256 _amount, address _tokenAddress) internal returns (bool) {
        require(hasRole(AUTHORIZED_CALLER_ROLE, _msgSender()), "AssetTokenAuth: only AUTHORIZED_CALLER");
        IPermissionManager permissionManager = IPermissionManager(permissionManagerAddress);
    ...
    function isTxAuthorized(
        ...
        require(assetTokenFactory.isTokenEnabled(_tokenAddress), "AssetTokenAuth: tokenAddress is not enabled");
    ...
    function setValidationAsSecurityAndAmount(
        ...
        if (_validateAsSecurityToken) {
            require(_fiatAmountValidation > 0, "AssetTokenAuth: fiatAmountValidation must be > 0");
        } else {
            require(_fiatAmountValidation == 0, "AssetTokenAuth: fiatAmountValidation must be 0");
        }
