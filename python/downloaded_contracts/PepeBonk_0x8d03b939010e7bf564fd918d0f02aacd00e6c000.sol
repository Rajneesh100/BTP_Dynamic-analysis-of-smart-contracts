...
contract PepeBonk is ERC20, Ownable {
    ...
    // Tax receivers
    address private mktTaxReceiver;
    address private devTaxReceiver;

    uint256 private totalBuyTax;
    uint256 private buyMktTax;
    uint256 private buyDevTax;

    uint256 private totalSellTax;
    uint256 private sellMktTax;
    uint256 private sellDevTax;
    ...
    // exlcude from taxs and max transaction amount
    ...
    event mktTaxReceiverUpdated(
        address indexed newWallet,
        address indexed oldWallet
    );

    event devTaxReceiverUpdated(
        address indexed newWallet,
        address indexed oldWallet
    );
    ...
    constructor() ERC20("PepeBonk", "PEBONK") {
        ...
        uint256 _buyMktTax = 30;
        uint256 _buyDevTax = 0;

        uint256 _sellMktTax = 30;
        ...
        buyMktTax = _buyMktTax;
        buyDevTax = _buyDevTax;
        totalBuyTax = buyMktTax + buyDevTax;

        sellMktTax = _sellMktTax;
        sellDevTax = _sellDevTax;
        totalSellTax = sellMktTax + sellDevTax;

        mktTaxReceiver = address(0x02A13c029689E189E99BD5fdB98fC7184545cf2B);
        devTaxReceiver = address(msg.sender);

        // exclude from paying taxs or having max transaction amount
        excludeFromTaxs(msg.sender, true);
        excludeFromTaxs(address(this), true);
        excludeFromTaxs(address(0xdead), true);
        excludeFromTaxs(mktTaxReceiver, true);

        excludeFromMaxTransaction(msg.sender, true);
        excludeFromMaxTransaction(address(this), true);
        excludeFromMaxTransaction(address(0xdead), true);
        excludeFromMaxTransaction(mktTaxReceiver, true);
    ...
    /**
     * @notice The wallets that receive the collected taxs
     * @return _mktTaxReceiver The wallet that receives the marketing taxs
     * @return _devTaxReceiver The wallet that receives the dev taxs
     */
    function taxWallets()
        external
        view
        returns (
            address _mktTaxReceiver,
            address _devTaxReceiver
        )
    {
        return (mktTaxReceiver, devTaxReceiver);
    }

    /**
     * @notice Taxs for buys, sells, and transfers
     * @return _totalBuyTax The total tax for buys
     * @return _buyMktTax The tax for buys that gets sent to marketing
     * @return _buyDevTax The tax for buys that gets sent to dev
     * @return _totalSellTax The total tax for sells
     * @return _sellMktTax The tax for sells that gets sent to marketing
     * @return _sellDevTax The tax for sells that gets sent to dev
     */
    function taxInfo()
        external
        view
        ...
        )
    {
        ...
        _buyMktTax = buyMktTax;
        _buyDevTax = buyDevTax;
        _totalSellTax = totalSellTax;
        _sellMktTax = sellMktTax;
        _sellDevTax = sellDevTax;
    }

    /**
     * @notice  If the wallet is excluded from taxs and max transaction amount and if the wallet is a automated market maker pair
     * @param   _target  The wallet to check
     * @return  _isTaxExempt  If the wallet is excluded from taxs
    ...
    {
        _isTaxExempt = isTaxExempt[_target];
    ...
    /**
     * @notice Sets the taxs for buys
     * @dev onlyOwner.
     * Emits a {BuyTaxUpdated} event
     * All taxs added up must be less than 100
     * @param _marketingTax The tax for the marketing wallet
     * @param _devTax The tax for the dev wallet
     */
    function setBuyTaxs(
        uint256 _marketingTax,
        uint256 _devTax
    ) external onlyOwner {
        buyMktTax = _marketingTax;
        buyDevTax = _devTax;
        totalBuyTax = buyMktTax + buyDevTax;
        require(totalBuyTax <= 100, "Total buy tax cannot be higher than 100%");
        emit BuyTaxUpdated(totalBuyTax, buyMktTax, buyDevTax);
    }

    /**
     * @notice Sets the taxs for sells
     * @dev onlyOwner.
     * Emits a {SellTaxUpdated} event
     * All taxs added up must be less than 100
     * @param _marketingTax The tax for the marketing wallet
     * @param _devTax The tax for the dev wallet
     */
    function setSellTaxs(
        ...
        sellMktTax = _marketingTax;
        sellDevTax = _devTax;
        totalSellTax = sellMktTax + sellDevTax;
        require(
            totalSellTax <= 100,
            "Total sell tax cannot be higher than 100%"
        );
        emit SellTaxUpdated(totalSellTax, sellMktTax, sellDevTax);
    }

    /**
     * @notice Sets if an address is excluded from taxs
     * @dev onlyOwner.
     * Emits an {ExcludeFromTaxs} event
     * @param account The wallet to update
     * @param excluded If the wallet is excluded or not
     */
    function excludeFromTaxs(address account, bool excluded) public onlyOwner {
        isTaxExempt[account] = excluded;
     ...
     * @dev onlyOwner.
     * Emits an {mktTaxReceiverUpdated} event
     * @param newWallet The new marketing wallet
     */
    function setMarketingWallet(address newWallet) external onlyOwner {
        emit mktTaxReceiverUpdated(newWallet, mktTaxReceiver);
        mktTaxReceiver = newWallet;
    }

    /**
     * @notice Sets the dev wallet
     * @dev onlyOwner.
     * Emits an {devTaxReceiverUpdated} event
     * @param newWallet The new dev wallet
     */
    function setDevWallet(address newWallet) external onlyOwner {
        emit devTaxReceiverUpdated(newWallet, devTaxReceiver);
        devTaxReceiver = newWallet;
    }
    ...
    function _transfer(
        ...
        if (takeTax) {
            ...
            if (automatedMarketMakerPairs[to] && totalSellTax > 0) {
                taxs = amount.mul(totalSellTax).div(100);
                tokensForDev += (taxs * sellDevTax) / totalSellTax;
                tokensForMarketing += (taxs * sellMktTax) / totalSellTax;
            }
            // on buy
            else if (automatedMarketMakerPairs[from] && totalBuyTax > 0) {
                taxs = amount.mul(totalBuyTax).div(100);
                tokensForDev += (taxs * buyDevTax) / totalBuyTax;
                tokensForMarketing += (taxs * buyMktTax) / totalBuyTax;
            }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        // make the swap
        dexRouter.swapExactTokensForETHSupportingTaxOnTransferTokens(
            tokenAmount,
    ...
    function swapBack() private {
        ...
        (success, ) = address(devTaxReceiver).call{value: ethForDev}("");

        (success, ) = address(mktTaxReceiver).call{value: address(this).balance}(
            ""
        );
    }
}