...
Channel - https://t.me/MagnumAnnouncements
...
contract Magnum is ERC20, Ownable {
    ...
    // Fee receivers
    address private MAGmktReceiver;
    address private devReceiver;

    uint256 private totalBuyFee;
    uint256 private buyMAGmktFee;
    uint256 private buyDevFee;

    uint256 private totalSellFee;
    uint256 private sellMAGmktFee;
    ...
    event BuyFeeUpdated(
        uint256 totalBuyFee,
        uint256 buyMAGmktFee,
        uint256 buyDevFee
    );

    event SellFeeUpdated(
        uint256 totalSellFee,
        uint256 sellMAGmktFee,
        uint256 sellDevFee
    );
    ...
    constructor() ERC20("Magnum", "MAG") {
        ...
        uint256 _buyMAGmktFee = 3;
        uint256 _buyDevFee = 0;

        uint256 _sellMAGmktFee = 5;
        uint256 _sellDevFee = 0;

        uint256 totalSupply = 3380000 * 10 ** decimals();
        ...
        buyMAGmktFee = _buyMAGmktFee;
        buyDevFee = _buyDevFee;
        totalBuyFee = buyMAGmktFee + buyDevFee;

        sellMAGmktFee = _sellMAGmktFee;
        sellDevFee = _sellDevFee;
        totalSellFee = sellMAGmktFee + sellDevFee;

        MAGmktReceiver = address(0x8C0B3CFdae38f7239a7372407251Fb13fA595016);
        ...
        excludeFromFees(MAGmktReceiver, true);

        excludeFromMaxTransaction(msg.sender, true);
        excludeFromMaxTransaction(address(this), true);
        excludeFromMaxTransaction(address(0xdead), true);
        excludeFromMaxTransaction(MAGmktReceiver, true);
     ...
     * @return _MAGmktReceiver The wallet that receives the marketing fees
    ...
    function receivers()
        ...
        view
        returns (
            address _MAGmktReceiver,
            address _devReceiver
        )
    {
        return (MAGmktReceiver, devReceiver);
    }

    /**
     * @notice Fees for buys, sells, and transfers
     * @return _totalBuyFee The total fee for buys
     * @return _buyMAGmktFee The fee for buys that gets sent to marketing
     * @return _buyDevFee The fee for buys that gets sent to dev
     * @return _totalSellFee The total fee for sells
     * @return _sellMAGmktFee The fee for sells that gets sent to marketing
    ...
    function txTaxes()
        ...
        returns (
            ...
            uint256 _buyMAGmktFee,
            uint256 _buyDevFee,
            uint256 _totalSellFee,
            uint256 _sellMAGmktFee,
            uint256 _sellDevFee
        )
    {
        _totalBuyFee = totalBuyFee;
        _buyMAGmktFee = buyMAGmktFee;
        _buyDevFee = buyDevFee;
        _totalSellFee = totalSellFee;
        _sellMAGmktFee = sellMAGmktFee;
    ...
    function setBuyFees(
        ...
        buyMAGmktFee = _marketingFee;
        buyDevFee = _devFee;
        totalBuyFee = buyMAGmktFee + buyDevFee;
        require(totalBuyFee <= 50, "Total buy fee cannot be higher than 50%");
        emit BuyFeeUpdated(totalBuyFee, buyMAGmktFee, buyDevFee);
    }
    ...
    function setSellFees(
        ...
        sellMAGmktFee = _marketingFee;
        sellDevFee = _devFee;
        totalSellFee = sellMAGmktFee + sellDevFee;
        require(
            totalSellFee <= 50,
            "Total sell fee cannot be higher than 50%"
        );
        emit SellFeeUpdated(totalSellFee, sellMAGmktFee, sellDevFee);
    }
     ...
     * @dev onlyOwner.
     * Emits an {MAGmktReceiverUpdated} event
     * @param newWallet The new marketing wallet
     */
    function setMarketingWallet(address newWallet) external onlyOwner {
        emit MAGmktReceiverUpdated(newWallet, MAGmktReceiver);
        MAGmktReceiver = newWallet;
    }
    ...
    function enableswap (bool enable) private onlyOwner {
        tradingLive = enable;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to] && totalSellFee > 0) {
                ...
                tokensForMarketing += (fees * sellMAGmktFee) / totalSellFee;
            }
            // on buy
                ...
                tokensForMarketing += (fees * buyMAGmktFee) / totalBuyFee;
            }
    ...
    function swapBack() private {
        ...
        (success, ) = address(MAGmktReceiver).call{value: address(this).balance}(
            ""
        );
    }
}