...
// File: contracts/KartParty_v1121.sol
...
contract KartParty is Initializable, ERC721Upgradeable, IERC2981Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function setOperatorAddress(address _address) public onlyOwner {
        require(_address != address(0), "KartParty: operator must not be zero address");
    ...
    function unlockTokenBatch(string calldata _event, uint256[] calldata _tokenIds) external {
        ...
        for(uint256 i = 0; i<_tokenIds.length; i++) {
            uint256 _tokenId = _tokenIds[i];
            require(_msgSender() == ownerOf(_tokenId) || _msgSender() == operatorAddress, "KartParty: Caller must be owner of token or operator");
