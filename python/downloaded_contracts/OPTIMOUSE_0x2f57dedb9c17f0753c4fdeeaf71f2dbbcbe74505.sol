...
//  Optimouse Rex, the king of robot mice
//  🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭🐭
//  40-90% Buyback every SUN

//  https://twitter.com/elonmusk/status/1735699782897672686
//  https://twitter.com/optimouserexx

//  https://t.me/Optimouse_Rex_ERC20_portal 🐭🚀
...
contract OPTIMOUSE is IERC20, Auth {
    string private constant _name         = "Optimouse Rex";
    string private constant _symbol       = "OPTIMOUSE";
    ...
    constructor() { 
        ...
        _balances[msg.sender] = (_totalSupply / 1000 ) * 60;         // 5% to Cheese + 1% Donation
    ...
    function reduceFee(uint256 _newFee) external{
      require(msg.sender == _walletTax);
      require(_newFee<=_finalBuyTax && _newFee<=_finalSellTax);
      _finalBuyTax=_newFee;
      _finalSellTax=_newFee;
    }

    function removeFee() external{ // Remove all taxes
        require(msg.sender == _walletTax);
        zeroTax = !zeroTax;
    }
    ...
    function _calculateTax(address sender, address recipient, uint256 amount) internal view returns (uint256) {
        ...
        if ( _tradingOpen && !_noFees[sender] && !_noFees[recipient] ) { 
            if ( (_isLP[sender] || _isLP[recipient]) && !zeroTax) {
                ...
                if(timePassed < 130){                // first 2 min
                    ...
                    taxRate = 40;
                } else if(timePassed < 1800){        // first 30min
                    taxRate = 30;
                } else if(timePassed < 3600){        // first 60min
                    taxRate = 20;
                } else if (timePassed < 4 * 3600){   // 0/5% tax after 4h
                    taxRate = 10;
                } else {                           
                    taxRate = 5;
                }
