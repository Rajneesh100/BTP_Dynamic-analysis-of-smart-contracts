...
abstract contract WithdrawAndDeposit is LargeOracleUpgradeable, Initializable {
    ...
    uint public depositMinimum;
    uint public depositMaximum;

    uint public withdrawSum;
    uint public withdrawMaximum;
    ...
    mapping (uint => bytes32) public processedWithdrawals;
    ...
    function _initialize(uint _depositMinimum,
                        uint _depositMaximum,
                        uint _withdrawMaximum,
    ...
    function update(uint64 newLargeOracleNonce,
        ...
        uint keySize = 64;
        uint noOfKeys = oracleKeys.length / keySize;
    ...
    function _deposit(bytes21 destination, uint amount) internal {
    ...
    function withdraw(uint64 withdrawNonce,
        ...
        uint withdrawId = (uint(oracleNonce) << 64) + uint(withdrawNonce);
    ...
    function withdrawFromPastEpoch(uint64 nonce,
        ...
        uint withdrawId = uint(epochOracleNonce) << 64 + uint(nonce);
    ...
    function _sendValue(address destination, uint amount) internal virtual;
...
contract WithdrawAndDepositErc20 is WithdrawAndDeposit {
    ...
    function deposit(bytes21 destination, uint amount) external {
        token.safeTransferFrom(msg.sender, address(this), amount);
        _deposit(destination, amount);
    }

    function _sendValue(address destination, uint amount) internal override {
