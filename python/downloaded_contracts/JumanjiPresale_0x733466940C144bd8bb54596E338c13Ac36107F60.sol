...
contract JumanjiPresale is Claimable {
    ...
    event ClaimBSC(address to, uint256 amount);
    ...
    function buyWithUSDT(uint256 usdtPrice) external returns (bool) {
        uint256 amount = usdtBuyHelper(usdtPrice);
        totalSaled += amount;

        uint256 ourAllowance = IERC20(USDTInterface).allowance(_msgSender(), address(this));

        require(usdtPrice <= ourAllowance, "Make sure to add enough allowance");

        require(IERC20(USDTInterface).transferFrom(_msgSender(),owner(), usdtPrice), "USDT transfer failed");
