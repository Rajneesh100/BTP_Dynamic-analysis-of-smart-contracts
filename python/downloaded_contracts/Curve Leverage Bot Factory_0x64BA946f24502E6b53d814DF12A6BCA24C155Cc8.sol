...
@payable
@nonreentrant('lock')
def create_bot(swap_infos: DynArray[SwapInfo, MAX_SIZE], collateral: address, debt: uint256, N: uint256, callbacker: address, callback_args: DynArray[uint256,5], leverage: uint256, deleverage_percentage: uint256, health_threshold: uint256, expire: uint256, number_trades: uint256, interval: uint256, bot_count: uint256):
    assert number_trades > 0, "Wrong number_trades"
    ...
    if bot_count > 1:
        ...
        self.bot_info[_deposit_id] = BotInfo({
            ...
            expire: expire,
            remaining_count: unsafe_sub(bot_count, 1)
        })
    else:
        assert bot_count == 1, "Wrong bot count"
    self._create_bot(_deposit_id, msg.sender, collateral, unsafe_div(collateral_amount, bot_count), debt, N, callbacker, callback_args, leverage, deleverage_percentage, health_threshold, expire, bot_count)

@internal
def _create_bot(deposit_id: uint256, depositor: address, collateral: address, amount: uint256, debt: uint256, N: uint256, callbacker: address, callback_args: DynArray[uint256,5], leverage: uint256, deleverage_percentage: uint256, health_threshold: uint256, expire: uint256, remaining_count: uint256):
    _service_fee: uint256 = self.service_fee
    controller: address = ControllerFactory(CONTROLLER_FACTORY).get_controller(collateral)
    ...
    if amount > 0:
        ...
        log BotStarted(deposit_id, depositor, bot, collateral, amount, debt, N, leverage, deleverage_percentage, health_threshold, expire, callbacker, callback_args, remaining_count)
...
def create_next_bot(deposit_id: uint256, callbacker: address, callback_args: DynArray[uint256,5], remaining_count: uint256):
    ...
    self._create_bot(deposit_id, _bot_info.depositor, _bot_info.collateral, _bot_info.amount, _bot_info.debt, _bot_info.N, callbacker, callback_args, _bot_info.leverage, _bot_info.deleverage_percentage, _bot_info.health_threshold, _bot_info.expire, remaining_count)
