...
// File: contracts/BIFI/strategies/Silo/StrategySilo.sol
...
contract StrategySilo is StratFeeManagerInitializable {
    ...
    address public constant output = 0xD533a949740bb3306d119CC777fa900bA034cd52;
    address public constant curvePool = 0x8301AE4fc9c624d1D396cbDAa1ed877821D7C511;
    address public constant crvUSDPool = 0x4eBdF703948ddCEA3B11f675B4D1Fba9d2414A14;
    address public want;
    address public silo;
    address public collateralToken;
    address[] public rewardsClaim;
    ISiloRewards public constant rewards = ISiloRewards(0x361384A0d755f972E5Eea26e4F4efBAf976B6461);
    ISiloLens public constant siloLens = ISiloLens(0x32a4Bcd8DEa5E18a12a50584682f8E4B77fFF2DF);
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        if (toNative > 0) ICurveSwap(curvePool).exchange(1, 0, toNative, 0);
    ...
    function swapToWant() internal {
        uint256 bal = IERC20(output).balanceOf(address(this));
        if (output != native) ICurveSwap(crvUSDPool).exchange(2, 0, bal, 0);
    }
    ...
    function _giveAllowances() internal {
        IERC20(output).approve(curvePool, type(uint).max);
        IERC20(output).approve(crvUSDPool, type(uint).max);
        IERC20(want).approve(silo, type(uint).max);
    }

    function _removeAllowances() internal {
        IERC20(output).approve(curvePool, 0);
        IERC20(output).approve(crvUSDPool, 0);
