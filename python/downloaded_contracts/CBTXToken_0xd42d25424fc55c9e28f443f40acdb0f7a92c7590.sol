...
// File: contracts/nove/CBTXToken.sol
...
contract CBTXToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public swapTokensAtAmount = 10_000_000_000 * (10**18);

    uint256 public buyFee = 10;
    uint256 public saleFee = 15;
    ...
    constructor(address _router, address _devAddress, address recipient) public ERC20("CBTX Token", "CBTX") {
    ...
    function setFee(uint256 _buyFee, uint256 _saleFee) external onlyOwner{
        buyFee = _buyFee;
        saleFee = _saleFee;
    }
    ...
    function _transfer(
        ...
        if(takeFee ) {
            ...
            uint256 fees = isBuy? amount.mul(buyFee).div(100) : amount.mul(saleFee).div(100);
            if(fees > 0) {
    ...
    function processFee(uint256 contractTokenBalance) private {
        ...
        swapTokensForEth(convertAmount);
        uint256 ethOut = address(this).balance - ethBal;

        payable(devAddress).transfer(ethOut);
    }
