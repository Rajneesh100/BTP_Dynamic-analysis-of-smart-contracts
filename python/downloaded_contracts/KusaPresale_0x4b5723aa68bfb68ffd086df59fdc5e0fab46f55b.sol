...
contract KusaPresale is Ownable {
    uint256 public constant PRESALE_ALLOCATION = 50_000_000;

    // Main token Contract
    IERC20 public tokenContract;

    // Phase 1 sale state and parameters
    bool public phase1State = false;
    uint256 public phase1Cap = 0 ether;
    uint256 public phase1TotalDeposits = 0;
    uint256 public phase1MaxDeposit = 0 ether;
    uint256 public phase1MinDeposit = 0 ether;
    mapping(address => uint256) private phase1UserBalance;
    bytes32 public phase1MerkleRoot;

    // Phase 2 sale state and parameters
    bool public phase2State = true;
    uint256 public phase2Cap = 28 ether;
    uint256 public phase2TotalDeposits = 0;
    uint256 public phase2MaxDeposit = 0.5 ether;
    ...
    function getKusa(uint256 investment) internal view returns (uint256) {
        uint256  PRICE_PER_TOKEN = ((phase2TotalDeposits + phase1TotalDeposits) * 1e18) / PRESALE_ALLOCATION;
        return investment / PRICE_PER_TOKEN;
    }

    function getKusaOwed(address user) external view returns (uint256) {
        uint256 totalContributions = phase1UserBalance[user] + phase2UserBalance[user];
    ...
    function claimTokens() public {
        ...
        uint256 totalContributions = phase1UserBalance[msg.sender] + phase2UserBalance[msg.sender];
    ...
    function getUserDepositedAmount(address _user, uint256 _phase) public view returns (uint256) {
        if (_phase == 1) {
            ...
            return  phase1UserBalance[_user] +  phase2UserBalance[_user];
        }
    }
