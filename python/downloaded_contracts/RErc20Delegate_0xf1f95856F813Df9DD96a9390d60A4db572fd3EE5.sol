// File: contracts/tbills/IKycManager.sol
 ...
 * @title IKycManager
 * @author River
 * @notice The interface for River's KYC Manager contract
 */
...
// File: contracts/tbills/RiverKYCStorage.sol
...
// File: contracts/delegateGeneric/RTokenInterfacesModified.sol
...
abstract contract RTokenInterface is RTokenStorage, RiverKYCStorage {
    /**
...
abstract contract RErc20Interface is RErc20Storage {
    ...
    function liquidateBorrow(
        address borrower,
        uint repayAmount,
        RTokenInterface cTokenCollateral
    ) external virtual returns (uint);
...
// File: contracts/delegateGeneric/RTokenModified.sol
...
/**
 * @title River's CToken Contract
 * @notice Abstract base for CTokens
 * @author Compound + River
 */
abstract contract RTokenModified is
    RTokenInterface,
    ExponentialNoError,
    TokenErrorReporter
{
     ...
     * @param kycManager_ The address of the KYC manager
     */
    function initialize(
    ...
    function mintFresh(address minter, uint mintAmount) internal {
        kycManager.onlyNotBanned(minter);
    ...
    function redeemFresh(
        ...
        kycManager.onlyNotBanned(redeemer);
        require(
    ...
    /**
     * @notice Update KYC manager address
     *
     * @param kycManager The new KYC manager address
     */
    function setKYCManager(address kycManager) external {
        require(msg.sender == admin, "Only admin can set KYC manager");
        _setKYCManager(kycManager);
    }

    /**
     * @notice Sets the KYC manager address for this client
     * @param _kycManager The new KYC manager address
     */
    function _setKYCManager(address _kycManager) internal {
        require(
            _kycManager != address(0),
            "KYC manager cannot be zero address"
        );
        address oldKYCManager = address(_kycManager);
        kycManager = IKycManager(_kycManager);
        emit SetKYCManager(oldKYCManager, _kycManager);
    }
     ...
     */
    function _onlyValidKyc(address _account) internal view {
        kycManager.onlyKyc(_account);
        kycManager.onlyNotBanned(_account);
    }
...
// File: contracts/delegateGeneric/RErc20.sol
...
/**
 * @title River's CErc20 Contract
 * @notice CTokens which wrap an EIP-20 underlying
 * @author Compound + River
 */
contract RErc20 is RTokenModified, RErc20Interface {
    /**
...
// File: contracts/delegateGeneric/RErc20Delegate.sol
