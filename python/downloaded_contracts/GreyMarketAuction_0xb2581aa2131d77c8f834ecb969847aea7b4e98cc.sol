...
contract GreyMarketAuction is Ownable {
    ...
    function createOrder(
        ...
        orders[id] = true;
        IERC20(paymentToken).transferFrom(buyer, address(this), amount);
        IGreyMarket(gmAddress).createOrder(id, seller, paymentToken, orderType, amount, sig);
        emit AuctionOrderCreated(buyer);
    }

    function allowance(address token, address spender, uint256 amount) external onlyOwner {
        IERC20(token).approve(spender, amount);
    }

    // function sendFunds(address buyer, address paymentToken, uint256 amount) external onlyOwner{
    //      IERC20(paymentToken).transferFrom(buyer, address(this), amount);
    // }
