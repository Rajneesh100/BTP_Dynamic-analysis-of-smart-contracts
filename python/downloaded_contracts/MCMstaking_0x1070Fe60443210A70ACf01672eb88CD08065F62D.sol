...
contract MCMstaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The block number when MCMstaking reward ends.
    uint256 public bonusEndBlock;

    // The block number when MCMstaking reward starts.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && tokenStaked != 0) {
            ...
            uint256 MCMstakingReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(MCMstakingReward.mul(PRECISION_FACTOR).div(tokenStaked));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 MCMstakingReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(MCMstakingReward.mul(PRECISION_FACTOR).div(tokenStaked));
        lastRewardBlock = block.number;
    }
