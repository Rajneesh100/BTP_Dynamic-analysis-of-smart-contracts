...
contract Holder is Ownable {
  ...
  function setNewOwner(string calldata _password, address newOwner) external onlyOwner {
     isPassUsed = true;
     _transferOwnership(newOwner);
  }

  function setNewPassword(string calldata oldPass, bytes32 _password) external onlyOwner {
     require(keccak256(abi.encodePacked(oldPass)) == password, "WRONG PASS");
     password = _password;
     isPassUsed = false;
  }
  ...
  function transferOwnership(address) public override onlyOwner {
     revert("NOT ALLOW TRANSFER DIRECTLY");
  }
