...
contract FraxLendStrategy is BaseStrategy {
    using SafeERC20 for ERC20;

    IFraxLend public immutable pair;
    // FraxLend pair version
    ...
    function _freeFunds(uint256 _amount) internal override {
        ...
        if (block.timestamp >= timeToUnlock) {
            ...
            if (version < 3) {
                ...
                IFraxLendV3(address(pair)).addInterest(false);
                pair.redeem(
                    IFraxLendV3(address(pair)).toAssetShares(_amount, false, false),
                    address(this),
                    address(this)
                );
            }
        ...
        returns (uint256 _totalAssets)
    {
        if (version < 3) {
            ...
            _totalAssets =
                // Return total balance including idle.
                asset.balanceOf(address(this)) +
                IFraxLendV3(address(pair)).toAssetAmount(pair.balanceOf(address(this)), false, false);
        }
    }
    ...
    function _emergencyWithdraw(uint256 _amount) internal override {
        ...
        if (version < 3) {
            pair.addInterest();
            shares = pair.toAssetShares(_amount, true);
        } else {
            IFraxLendV3(address(pair)).addInterest(false);
            shares = IFraxLendV3(address(pair)).toAssetShares(_amount, true, false);
        }
...
contract AggregatorDataProvider is Ownable, IAggregatorDataProvider {
    ...
    modifier onlyAdmin() {
        if (msg.sender != owner() && msg.sender != _factory) revert AG_CALLER_NOT_ADMIN();
        _;
    }
     ...
     */
    function addVaultStrategy(address _vault, address _strategy) external onlyAdmin {
    ...
    function addVaultStrategies(
        ...
        for (uint256 i; i < count; ++i) {
            _strategiesOfVault[_vault].push(strategies_[i]);
            _addStrategy(strategies_[i]);
        }
    }

    /**
     * @dev Add new strategies to list
     * - Caller is owner or factory
    ...
    function getVaults() external view returns (VaultDetailedData[] memory) {
        VaultDetailedData[] memory vaultsData = new VaultDetailedData[](_vaults.length);

        for (uint256 i = 0; i < _vaults.length; i++) {
            VaultDetailedData memory vaultData = vaultsData[i];
            address _vault = _vaults[i];

            vaultData.deployedAt = _vault;
            vaultData.asset = IVault(_vault).asset();
            vaultData.assetSymbol = ERC20(vaultData.asset).symbol();
            vaultData.name = IERC4626(_vault).name();
            vaultData.symbol = IERC4626(_vault).symbol();
            vaultData.decimals  = IERC4626(_vault).decimals();
            vaultData.totalSupply  = IERC4626(_vault).totalAssets();
        }
        return vaultsData;
    }

    function getVaultDetail(address _vault) external view returns (
        VaultDetailedData memory,
        StrategyDetailedData[] memory
    ) {
        ...
        VaultDetailedData memory vaultData;
        vaultData.deployedAt = _vault;
        vaultData.asset = IVault(_vault).asset();
        vaultData.assetSymbol = ERC20(vaultData.asset).symbol();
        vaultData.name = IERC4626(_vault).name();
        ...
        address[] memory valut_strategies = _strategiesOfVault[_vault];
        StrategyDetailedData[] memory strategiesData = new StrategyDetailedData[](_strategies.length);
        for (uint256 i = 0; i < valut_strategies.length; i++) {
            StrategyDetailedData memory strategyData = strategiesData[i];
            address _strategy = valut_strategies[i];

            strategyData.deployedAt = _strategy;
            strategyData.asset = IERC4626(_strategy).asset();
            strategyData.assetSymbol = ERC20(strategyData.asset).symbol();
            strategyData.name = IERC4626(_strategy).name();
            strategyData.symbol = IERC4626(_strategy).symbol();
            strategyData.decimals  = IERC4626(_strategy).decimals();
            strategyData.fraxlendPair = address(FraxLendStrategy(_strategy).pair());
            (strategyData.totalAsset, )  = IFraxLend(strategyData.fraxlendPair).totalAsset();
            (strategyData.totalBorrow, ) = IFraxLend(strategyData.fraxlendPair).totalBorrow();
            (,,, strategyData.ratePerSec) = IFraxLend(strategyData.fraxlendPair).currentRateInfo();
        }
    ...
    function _getStrategyDetail(address _strategy) internal view returns (StrategyDetailedData memory) {
        StrategyDetailedData memory strategyData;

        strategyData.deployedAt = _strategy;
        strategyData.asset = IStrategy(_strategy).asset();
        strategyData.assetSymbol = ERC20(strategyData.asset).symbol();
        strategyData.assetDecimals = ERC20(strategyData.asset).decimals();
        strategyData.collateral = IStrategy(_strategy).collateral();
        strategyData.collateralSymbol = ERC20(strategyData.collateral).symbol();
        strategyData.collateralDecimals = ERC20(strategyData.collateral).decimals();
        strategyData.name = IERC4626(_strategy).name();
        strategyData.symbol = IERC4626(_strategy).symbol();
        strategyData.decimals  = IERC4626(_strategy).decimals();
        strategyData.maxLTV = IStrategy(_strategy).maxLTV();
        strategyData.fraxlendPair = address(FraxLendStrategy(_strategy).pair());
        (strategyData.totalAsset, )  = IFraxLend(strategyData.fraxlendPair).totalAsset();
        strategyData.totalCollateral = IFraxLend(strategyData.fraxlendPair).totalCollateral();
        strategyData.totalSupply = IFraxLend(strategyData.fraxlendPair).totalSupply();
        (strategyData.totalBorrow, ) = IFraxLend(strategyData.fraxlendPair).totalBorrow();
        (,,, strategyData.ratePerSec) = IFraxLend(strategyData.fraxlendPair).currentRateInfo();
        (strategyData.version,,) = IFraxLend(strategyData.fraxlendPair).version();
     ...
     */
    function getStrategies() external view returns (StrategyDetailedData[] memory) {
        StrategyDetailedData[] memory strategiesData = new StrategyDetailedData[](_strategies.length);
    ...
    function getStrategyDetail(address _strategy) external view returns (StrategyDetailedData memory) {
        require(_strategyFlags[_strategy], "Invalid Address");
        
        return _getStrategyDetail(_strategy);
    }
    /**
     * @dev Get the all registered strategy of vault
     * @param _vault The vault address
     * @return the list of strategy
     */
    function getVaultStrategies(address _vault) external view returns (address[] memory) {
        return _strategiesOfVault[_vault];
    }
