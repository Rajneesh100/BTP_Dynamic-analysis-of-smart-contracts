...
contract FantasyDollarsNFT is Context, ERC165, IERC721, IERC721Metadata, Ownable {

    using Address for address;

    // Token name
    string private constant _name = "Fantasy Dollars NFT";

    // Token symbol
    string private constant _symbol = "FADN";

    // total number of NFTs Minted
    uint256 private _totalSupply;

    // total number of NFTs that can be minted
    uint256 public maxSupply = 2_500;

    // cost in ETH to mint 1 nft
    uint256 public mintCost = 0.05 ether;

    // discount minimum
    uint256 public discountMinimum = 10;
    ...
    // payable addresses
    address private constant addr0 = 0xbbe819578c6536Ba8BD0A01E9f4f521E70935af7;
    address public recipient = 0x61F728aA3cD0d15205E6E66F8d8b478Ad4D42A5D;
    uint256 private valueToSwitchToRecipient = 13 ether;
    uint256 private totalValueCollected;
    ...
    function setDiscountMin(uint newMin) external onlyOwner {
        discountMinimum = newMin;
    }
    ...
    function setValueToSwitch(uint newVal) external onlyOwner {
        valueToSwitchToRecipient = newVal;
    }
    ...
    function mint(uint256 qty) external payable {
        ...
        if (qty >= discountMinimum) {
            require(
                msg.value >= ( mintCost * qty * 9 ) / 10,
                'Insufficient Value'
            );
        } else {
            require(
        ...
        // divvy up value
        if (totalValueCollected <= valueToSwitchToRecipient) {
            (bool s,) = payable(addr0).call{value: address(this).balance}("");
            require(s, 'Transfer Fail');
        } else {
            (bool s,) = payable(recipient).call{value: address(this).balance}("");
            require(s, 'Transfer Recipient Fail');
        }

        // add to total value collected
        unchecked {
            totalValueCollected += msg.value;
        }
