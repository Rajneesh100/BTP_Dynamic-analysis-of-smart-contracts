...
// File contracts/UBDValidator.sol
...
contract UBDValidator is Ownable, Pausable {
    ...
    constructor(address _signer, address _usdt) Ownable(msg.sender) {
        _USDT = IERC20(_usdt);
        signer = _signer;
    }

    function claimReward(uint256 usdtAmount, uint32 nonce, bytes memory signature) external whenNotPaused {
        userNonce[msg.sender]++;

        bytes32 message = _prefixed(keccak256(abi.encodePacked(usdtAmount, msg.sender, nonce)));
        ...
        _USDT.transfer(msg.sender, usdtAmount);
    }
