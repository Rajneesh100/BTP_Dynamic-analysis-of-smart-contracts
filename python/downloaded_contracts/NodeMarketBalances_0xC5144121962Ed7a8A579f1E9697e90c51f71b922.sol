...
// File: contracts/new_deposit_share_test.sol
...
contract NodeMarketBalances is Ownable(msg.sender), ReentrancyGuard {
    ...
    function createDeposit(
        ...
        require(
            erc20coin.balanceOf(msg.sender) >= _depositAmount,
            "Insufficient USDC"
        );
        ...
        //deposits.push(newDeposit);
        IERC20(erc20coin).transferFrom(msg.sender, address(this), _depositAmount);
    }
    ...
    function changeDepositAmount(
        ...
        uint256 decimalNewDepositAmount = _depositAmount * (10**erc20decimals);

        if (current.depositAmount > _depositAmount) {
            IERC20(erc20coin).transfer(
                owner(),
                (current.depositAmount - decimalNewDepositAmount)
            );
        } else if (current.depositAmount < _depositAmount) {
            IERC20(erc20coin).transferFrom(
                msg.sender,
                address(this),
                (decimalNewDepositAmount - current.depositAmount)
            );
        }

        deposits[idx].depositAmount = decimalNewDepositAmount;
        deposits[idx].remaining = decimalNewDepositAmount;
        updateShare(depositId);
