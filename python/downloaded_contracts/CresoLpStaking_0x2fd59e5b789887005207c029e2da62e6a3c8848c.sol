...
contract CresoLpStaking is Ownable, ReentrancyGuard {
     ...
     */
    constructor() {
        lpToken = IERC20Metadata(0x7262a43c94258e6071d3bAC00eBeC6d7c9B4Ef30); //UniSwap v2 CRESO-ETH
    ...
    function recoverToken(address _token) external onlyOwner {
        ...
        require(_token != address(cresoToken), "Operations: Cannot recover reward token");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > lastRewardTimeStamp && totalStaked != 0) {
            ...
            uint256 totalStakedCresoEquivalent = 2 * cresoToken.balanceOf(address(lpToken)) * totalStaked / lpToken.totalSupply();
            uint256 estimatedReward = multiplier * (((totalStakedCresoEquivalent * apy)/100) / (365 days));
