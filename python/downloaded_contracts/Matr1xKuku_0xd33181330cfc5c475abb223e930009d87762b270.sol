...
contract Matr1xKuku is Initializable, ERC721Upgradeable, AccessControlUpgradeable, ERC2981Upgradeable, DefaultOperatorFiltererUpgradeable, UUPSUpgradeable {
    using SafeMathUpgradeable for uint256;

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant UPGRADER_ROLE = keccak256("UPGRADER_ROLE");

    uint256 private constant MAX_OWNER_NUM = 2000;
    uint256 private constant MAX_AIRDROP_NUM = 524;
    uint256 private constant MAX_KUKULIST_NUM = 3492;
    uint256 private constant MAX_KUKU_NUM = 6666;
    ...
    // 0-close 1-airdrop/kukuList 2-luckyList
    ...
    uint256 private _kukulistMinted;

    // Id -> 0/1 ( 0 - not minted, 1 - minted)
    mapping(string => uint256) private _adMinted;
    mapping(string => uint256) private _llMinted;

    // Id -> kukuListNum
    mapping(string => uint256) private _kukuListNum;
    // Id -> mintedNumber
    mapping(string => uint256) private _mintedNumber;

    event ContractStateEvent(uint256 saleState);
    event KukuMint(string id, string lable, address wallet, uint256 number, uint256 mintTime);
    // ============ mint end ===========
    ...
    function initialize() initializer public {
        __ERC721_init("MATR1X KUKU", "KUKU");
        ...
        _burnCounter = 0;
        state = 0;
        mintCost = 0.398 ether;
        _ownerTokenId = 1;
        _nextTokenId = 2001;
        _buri = "https://metadata.matr1x.io/metadata/kuku/";

        _walletAddr = 0x869Cb8fE8f7E0b1A483FEe1200e9F36176539f59;
        mintSigner = 0x8066c737B2c0c035255CDec0D1928Fa30026aF33;
    ...
    function setBaseURI(string memory buri) external onlyRole(MINTER_ROLE) {
    ...
    modifier checkState(uint8 saleState_) {
        require(state == saleState_, "Invalid Sale State");
        _;
    }

    modifier isEoaWallet() {
     ...
     * owner mint
     */
    function ownerMint(uint256 num, address to) public onlyRole(MINTER_ROLE) {
        uint256 tokenId = _ownerTokenId;
        require(tokenId - 1 + num <= MAX_OWNER_NUM, "Not enough tokens left to mint");
        ...
        _ownerTokenId = tokenId;
    }

    /**
     * airdrop mint
     */
    function adMint(string calldata _id, uint256 _accNum, bytes memory _signature)
    external
    checkState(1)
    isEoaWallet
    {
        address _signerOwner = signatureWallet(_msgSender(), _accNum, _accNum, _id, "kkadMint", _signature);
        // check signerOwner
        require(_signerOwner == mintSigner, "Not authorized to mint");
        require(_adMinted[_id] == 0, "Have been minted");

        unchecked{
            _airdropMinted += _accNum;
        }
        require(_airdropMinted <= MAX_AIRDROP_NUM, "Reached airdrop mint maximum limit");

        _adMinted[_id] = 1;

        _mintBatch(_msgSender(), _accNum);

        emit KukuMint(_id, "kkadMint", _msgSender(), _accNum, block.timestamp);
    }

    /**
     * kukulist mint
     */
    function kkwlMint(string calldata _id, uint256 _accNum, uint256 _accTotalNum, bytes memory _signature)
    external
    payable
    checkState(1)
    isEoaWallet
    {
        require(_accNum > 0 && _accTotalNum > 0, "The number must be greater than zero");

        address _signerOwner = signatureWallet(_msgSender(), _accNum, _accTotalNum, _id, "kkwlMint", _signature);
        // check signerOwner
        require(_signerOwner == mintSigner, "Not authorized to mint");

        if (_kukuListNum[_id] == 0) {
            _kukuListNum[_id] = _accTotalNum;
        }

        uint256 _num = _kukuListNum[_id] - _mintedNumber[_id];
        require(_num > 0, "You have reached your mint limit");
        ...
        unchecked{
            _kukulistMinted += _num;
        }
        require(_kukulistMinted <= MAX_KUKULIST_NUM, "Reached kuku list maximum limit");

        unchecked{
            _mintedNumber[_id] += _num;
        }

        uint256 _totalAmount = price(mintCost, _num);
        require(msg.value >= _totalAmount, "Value below price");

        uint256 _change;
        unchecked{
            _change = msg.value - _totalAmount;
        }
        ...
        _mintBatch(_msgSender(), _num);

        emit KukuMint(_id, "kkwlMint", _msgSender(), _num, block.timestamp);
    }

    /**
     * Lucklist mint
     */
    function kkllMint(string calldata _id, uint256 _accNum, bytes memory _signature)
    external
    payable
    checkState(2)
    isEoaWallet
    {
        address _signerOwner = signatureWallet(_msgSender(), _accNum, _accNum, _id, "kkllMint", _signature);
        // check signerOwner
        require(_signerOwner == mintSigner, "Not authorized to mint");
        require(_nextTokenId - 1 + _accNum <= MAX_KUKU_NUM, "Reached kuku mint maximum limit");
        require(_llMinted[_id] == 0, "Have been minted");

        uint256 _totalAmount = price(mintCost, _accNum);
        require(msg.value >= _totalAmount, "Value below price");

        _llMinted[_id] = 1;
        _mintBatch(_msgSender(), _accNum);

        emit KukuMint(_id, "kkllMint", _msgSender(), _accNum, block.timestamp);
    }
    ...
    function setStatus(uint256 _status) external onlyRole(MINTER_ROLE) {
        state = _status;
        emit ContractStateEvent(_status);
    }

    function setSigner(address _signer) external onlyRole(MINTER_ROLE) {
        mintSigner = _signer;
    }
    ...
    function currentIndex() public view returns (uint256, uint256, uint256){
        return (_airdropMinted, _kukulistMinted, _nextTokenId - 1);
    }


    function updateWalletAddress(address _address) external onlyRole(MINTER_ROLE) {
        _walletAddr = _address;
    }
    ...
    function withdrawAll() public onlyRole(MINTER_ROLE) {
        ...
        _widthdraw(_walletAddr, address(this).balance);
    }
    ...
    function signatureWallet(address _wallet, uint256 _numberType, uint256 _numberType1, string calldata _stringType, string memory lable, bytes memory _signature)
    public
    pure
    returns (address)
    {
        return ECDSAUpgradeable.recover(keccak256(abi.encode(_wallet, _numberType, _numberType1, _stringType, lable)), _signature);
    }
    ...
    function totalSupply() public view virtual returns (uint256) {
        return _nextTokenId - 1 - 2000 + _ownerTokenId - 1 - _burnCounter;
    }
    ...
    {
        require(_lockCounts[tokenId] == 0, "Token locked by event");
