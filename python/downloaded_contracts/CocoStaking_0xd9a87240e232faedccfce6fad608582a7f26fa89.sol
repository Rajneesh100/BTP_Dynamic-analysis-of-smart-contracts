...
contract CocoStaking is Context, Pausable, Ownable, ReentrancyGuard {
    ...
    uint256 public interestperDay = 7000000000;
    uint256 public withdrawalLockDuration = 50 days;
    ...
    function manageToken(uint256 amount, address tokenAddress)
        public
        onlyOwner
        nonReentrant
    {
        ...
        require(
            ...
            "Ownable: contract does not have enough tokens"
        );
        IERC20(tokenAddress).transfer(_msgSender(), amount);
    }

    function stakeForWallet(address _wallet, uint256 _amount)
        external
        onlyOwner
        nonReentrant
    {
        stakeprivate(_wallet, _amount);
    ...
    function setInterestperDay(uint256 _interestperDay)
        external
        onlyMultiSignature
    {
    ...
    function stakeprivate(address wallet, uint256 amount) private {
        ...
        if (YesNoStaked) {
            if (pend(wallet) >= holdingXstaking[wallet] / 1000) {
                revert(
    ...
    function resetMyRewards() external {
        rewards[msg.sender] = 0;
        paidRewards[msg.sender] = 0;
    ...
    function claimReward() external nonReentrant whenNotPaused {
        ...
        paidRewards[msg.sender] += yourrewards;
    ...
    {
        uint256 _return;
        if (((a / 100000000000)) / 100 >= paidRewards[account]) {
            _return = ((a / 100000000000)) / 100 - paidRewards[account];
        } else {
            _return = 0;
        }
        return _return;
