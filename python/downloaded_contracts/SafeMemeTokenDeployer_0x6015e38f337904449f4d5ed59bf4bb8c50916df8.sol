...
contract SafeMemeTokenDeployer is Ownable {
    address public deployedContractAddress;
    uint256 deployedTokenCounter=0;
    address public  devWallet=0x829ceb39FeE0155d63530de02450AbC3b6652602;
    address public revenueWallet=0x829ceb39FeE0155d63530de02450AbC3b6652602;

    uint256 deployFee=0.0005 ether;
    event NewContractDeployed(address indexed deployedAddress,string symbol,string tokenName,uint256 totalSupply);

    constructor()Ownable(msg.sender) {
    }

    function deployNewContract(uint256 _totalSupply,uint256 _initBuyTax,uint256 _initSellTax,string memory _tokenSymbol,string memory _tokenName,address marketingWallet,uint8 _decimal,uint256 finalSellTax,uint256 finalBuyTax) payable  public {
        require(msg.value>=deployFee,"insufficient fee");
        uint256 devFee=msg.value/10;
        uint256 revFee=msg.value-devFee;
        payable (devWallet).transfer(devFee);
        payable (revenueWallet).transfer(revFee);
        SafeToken newContract = new SafeToken(_totalSupply,_initBuyTax,_initSellTax,_tokenSymbol,_tokenName,marketingWallet,_decimal,finalSellTax,finalBuyTax,msg.sender);
        deployedContractAddress = address(newContract);
        deployedTokenCounter++;
        emit NewContractDeployed(deployedContractAddress,_tokenSymbol,_tokenName,_totalSupply);
    }

    function setFees(uint256 newFee) onlyOwner public  {
        deployFee=newFee;
    }

    function setNewDevWallet(address _devWallet) onlyOwner public {
devWallet=_devWallet;
    }
     function setNewRevWallet(address _revWallet) onlyOwner public {
revenueWallet=_revWallet;
    }
