...
// File contracts/token/ERC20FactoryByBankPad.sol
...
contract ERC20FactoryByBankPad is Ownable, Pausable {
    ...
    function _lockLP(
        ...
        require(lockPeriod >= MIN_LOCK_PERIOD, "too short lock");
    ...
    function viewLPLocker(address dexRouter) external view returns (address) {
    ...
    function viewMachine() external view returns (address) {
        return _machine;
    }

    /// @notice Update service fee receive account
    function updateServicePayAccount(
        address payable servicePayAccount
    ) external onlyOwner {
        require(servicePayAccount != address(0), "invalid pay account");
        // confirm service pay account can receive ETH
        servicePayAccount.sendValue(0);

        _servicePayAccount = servicePayAccount;
    }
