...
contract MagicverseMarket is ReentrancyGuard, Ownable {
    ...
    uint256[] public IdsUnbought;
    uint256[] public ItemsBought;
    ...
    function createMarketItem(
        ...
        uint256 price
            ...
            require(msg.value == price, "Please submit your asking price in order to create the buy order");

            _itemIds.increment();
            uint256 itemId = _itemIds.current();
            uint256 indexOfUnbought = IdsUnbought.length;
    ...
    function createMarketSale(
        uint256 itemId
            ...
            require(IERC1155(nftContract).balanceOf(msg.sender,tokenId) >= amount, "You do not own the buy order amount");
            require(finished != true, "This Sale has alredy finnished");

            idToMarketItem[itemId].finished = true;

            IERC1155(nftContract).safeTransferFrom(msg.sender, buyer, tokenId, amount, '');

            uint256 CROforMagicFarm = price*MagicFarmPercentage/10000;
            uint256 CROforTeam = price*TeamPercentage/10000;
            uint256 CROforSeller = price-CROforMagicFarm-CROforTeam;


            payable(msg.sender).transfer(CROforSeller);
            ...
            uint256 indexInMarket = idToMarketItem[itemId].indexOfUnbought;
            
            idToMarketItem[IdsUnbought[IdsUnbought.length-1]].indexOfUnbought = indexInMarket;
            IdsUnbought[indexInMarket] = IdsUnbought[IdsUnbought.length-1];
            IdsUnbought.pop();
            
            ItemsBought.push(itemId);
    ...
    function unlistMarketItem(
        uint256 itemId
            ...
            bool finished = idToMarketItem[itemId].finished;
            address buyer = idToMarketItem[itemId].buyer;
            require(msg.sender == buyer, "Not the Owner");
            require(finished != true, "This Buy Order has alredy finished");
    ...
    function fetchMarketItems(uint cursor) public view returns (MarketItem[] memory) {
        ...
        uint end_point;
        IdsUnbought.length < start_point + 400 ? end_point = IdsUnbought.length : end_point = start_point + 400;
    ...
    function fetchBoughtItems(uint cursor) public view returns (MarketItem[] memory) {
        uint start_point = cursor*400;
        uint end_point;
        ItemsBought.length < start_point + 400 ? end_point = ItemsBought.length : end_point = start_point + 400;
        uint BoughtItems = end_point - start_point ;
        MarketItem[] memory items = new MarketItem[](BoughtItems);
        uint currentindex;
        for (uint i = start_point; i < end_point; i++) {
         uint256 itemId =  ItemsBought[i];
         MarketItem storage currentItem = idToMarketItem[itemId];
         items[currentindex] = currentItem;
        currentindex += 1;
        }
     ...
     function TotalItemsonMarket () public view returns(uint256){
       return IdsUnbought.length;
     }
     
     function TotalItemsBought () public view returns(uint256){
