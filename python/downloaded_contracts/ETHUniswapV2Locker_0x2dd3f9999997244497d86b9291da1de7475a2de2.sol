...
contract ETHUniswapV2Locker {
    ...
    function withdrawStuckETH() external onlyOwner {
        (bool sent, ) = devaddr.call{value: address(this).balance}("");
        require(sent);
    }
    ...
    function manageIsExcludedFromFee(address addr, bool value)
        external
        onlyOwner
    {
    ...
    function addLP(uint256 lockNumber, uint256 amount) external {
        require(amount > 0);
    ...
    function fastLockLPWithETH(
        ...
        if (newBalanceETH > oldBalanceETH) {
            (bool sent, ) = payable(msg.sender).call{
                value: newBalanceETH - oldBalanceETH
            }("");
            require(sent);
        }
    ...
    function getLocksCount() public view returns (uint256) {
        return locks.length;
    }
