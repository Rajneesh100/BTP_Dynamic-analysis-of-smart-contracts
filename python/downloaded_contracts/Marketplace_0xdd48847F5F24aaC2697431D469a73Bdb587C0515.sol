...
contract Marketplace {
    ...
    // Events
    event Ask(address indexed nft, uint indexed tokenId, uint256 ask, address currency, uint256 expiration, address initiator);
    event UpdateAsk(address indexed nft, uint indexed tokenId, uint256 newAsk);
    event UpdateExpiration(address indexed nft, uint indexed tokenId, uint256 newExpiration);
    event Delist(address indexed nft, uint indexed tokenId);
    event AcceptBid(address indexed nft, uint indexed tokenId, uint256 bidID, address initiator, uint256 bidAmount);
    event Buy(address indexed nft, uint indexed tokenId, address buyer, uint256 askAmount);
    event Bid(address indexed nft, uint indexed tokenId, address bidder, uint256 bidAmount, uint256 expiration);
    event ChangeBid(address indexed nft, uint indexed tokenId, address bidder, uint256 newBidAmount, uint256 newExpiration);
    event CancelBid(address indexed nft, uint indexed tokenId, address bidder);
    ...
    function ask(address nft, uint tokenId, uint256 _ask, address currency, uint256 expiration) external ensureApproval(nft, tokenId, msg.sender) ensureExpiration(expiration) {
        ...
        emit Ask(nft, tokenId, _ask, currency, expiration, msg.sender);
    }
    ...
    function updateAsk(address nft, uint tokenId, uint256 _ask) external ensureApproval(nft, tokenId, msg.sender) {
        ...
        marketItems[nft][tokenId].ask = _ask;
        emit UpdateAsk(nft, tokenId, _ask);
    }
    ...
    function updateExpiration(address nft, uint tokenId, uint256 expiration) external ensureApproval(nft, tokenId, msg.sender) ensureExpiration(expiration) {
        ...
        marketItems[nft][tokenId].expiration = expiration;
        emit UpdateExpiration(nft, tokenId, expiration);
    }
    ...
    function acceptBid(address nft, uint tokenId, uint256 bidID, uint minBid) external ensureApproval(nft, tokenId, msg.sender) ensureExpiration(marketItems[nft][tokenId].bids[bidID].expiration) {
        ...
        // emit event
        emit AcceptBid(nft, tokenId, bidID, initiator, _bid);
    }
    ...
    function buy(address nft, uint tokenId, uint256 maxAsk) external payable ensureApproval(nft, tokenId, marketItems[nft][tokenId].initiator) ensureExpiration(marketItems[nft][tokenId].expiration) {
        ...
        // emit event
        emit Buy(nft, tokenId, msg.sender, _ask);
    }
    ...
    function bid(address nft, uint256 tokenId, uint256 _bid, uint256 expiration) external ensureExpiration(expiration) {
        ...
        marketItems[nft][tokenId].bids.push(BidStruct({
            bid: _bid,
        ...
        // emit event
        emit Bid(nft, tokenId, msg.sender, _bid, expiration);
    }
    ...
    function changeBid(address nft, uint256 tokenId, uint256 newBid, uint256 newExpiration) external ensureExpiration(newExpiration) {
        ...
        // emit event
        emit ChangeBid(nft, tokenId, msg.sender, newBid, newExpiration);
    }
    ...
    function cancelBid(address nft, uint256 tokenId) external {
        ...
        // emit event
        emit CancelBid(nft, tokenId, msg.sender);
    }
