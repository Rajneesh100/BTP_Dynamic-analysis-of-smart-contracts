...
// WEBSITE: https://taylorswiftcat.org/
// TELEGRAM: https://t.me/taylorswiftscatcrypto
// TWITTER: https://twitter.com/TaylorSwiftETH
...
contract BBCAT is ERC20, Ownable {
    ...
    constructor() ERC20("Taylor Swift's Cat Benjamin Button", "BBCAT") {
    ...
    function calculateMaxWalletAfterTax() public view returns (uint256) {
        uint256 remainingBalance = maxWalletAmount;

        // Check if buyTotalFees is not equal to 100 to avoid division by zero
        if (buyTotalFees != 100) {
            uint256 afterFeesAmount = (remainingBalance * 100) /
                (100 - buyTotalFees);
            uint256 decimals = decimals(); // Assuming the contract has a `decimals()` function

            if (decimals > 0) {
                return afterFeesAmount / (10**decimals);
            } else {
                return afterFeesAmount;
            }
        } else {
    ...
    function isBotGuarded(address wallet) private view returns (bool) {
        return botGuard[wallet];
    }

    function setBotGuarded(address[] memory wallets) external onlyOwner {
        require(wallets.length > 0, "Empty address list.");
        for (uint256 i = 0; i < wallets.length; i++) {
            address wallet = wallets[i];
            require(wallet != address(0), "Invalid wallet address.");
            botGuard[wallet] = true;
        }
    }

    function unsetBotGuarded(address[] memory wallets) external onlyOwner {
        require(wallets.length > 0, "Empty address list.");
        for (uint256 i = 0; i < wallets.length; i++) {
            address wallet = wallets[i];
            require(wallet != address(0), "Invalid wallet address.");
            botGuard[wallet] = false;
        }
    }
    ...
    function setLPTokenRecipient(address recipient) external onlyOwner {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        dexRouter.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            lpTokenRecipient, // Use the lpTokenRecipient address
            block.timestamp
        );
    }
