...
interface IMBPIDO {
    ...
    function getMinimumStakeDays() external view returns (uint256);
...
contract MobiPadRouter is Context, Ownable {
    ...
    function getMinimumStakeDays(address _ido_contract) external view returns (uint256){
        require(IS_IDO_CONTRACT[_ido_contract], "MBP: Not a MobiPad IDO Contract");
        ...
        return IDO_CONTRACT.getMinimumStakeDays();
    }
    ...
    function isWhitelisted(address _ido_contract, address _addr) external view returns (bool){
        ...
        return IDO_CONTRACT.isWhitelisted(_addr);
    }
